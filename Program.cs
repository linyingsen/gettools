...
{
    private static readonly Uri BaseUri = new("https://ai.codefather.cn");
    ...
    public static async Task Main(string[] args)
    {
        ...
        {
            Console.WriteLine("请先配置 SQL Server 连接串：");
            Console.WriteLine("1) 环境变量: AI_SQLSERVER_CONNECTION");
            Console.WriteLine("2) 或命令行参数: --conn \"Server=.;Database=ai;Trusted_Connection=True;TrustServerCertificate=True\"");
            return;
        }

        using var httpClient = BuildHttpClient();
        var crawler = new PromptCrawler(httpClient, BaseUri);

        var prompts = await crawler.CrawlAllAsync();
        Console.WriteLine($"共采集到 {prompts.Count} 条提示词（按唯一详情页去重后）。");
        ...
        Console.WriteLine($"数据库写入完成，影响行数：{affected}");
    }
    ...
    private static string? GetConnectionString(string[] args)
    {
        ...
        return Environment.GetEnvironmentVariable("AI_SQLSERVER_CONNECTION");
    }
}
...
internal sealed class PromptCrawler
{
    ...
    public PromptCrawler(HttpClient httpClient, Uri baseUri)
    {
    ...
    public async Task<List<PromptRecord>> CrawlAllAsync()
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine($"抓取列表页：{listUri}");

            var html = await TryGetHtmlAsync(listUri);
            if (string.IsNullOrWhiteSpace(html))
            {
                Console.WriteLine("列表页为空或访问失败，停止翻页。") ;
                break;
            }
            ...
            {
                Console.WriteLine("当前页未发现详情链接，停止翻页。") ;
                break;
            }

            Console.WriteLine($"列表页提取到 {detailUrls.Count} 个详情链接。") ;
    ...
    private Uri BuildListPageUri(int page)
    {
        return page <= 1
            ...
            : new Uri(_baseUri, $"/prompt?page={page}");
    }
    ...
    private async Task<string?> TryGetHtmlAsync(Uri uri)
    {
        ...
        {
            Console.WriteLine($"访问失败：{uri}，错误：{ex.Message}");
            return null;
        }
    }
    ...
    private IEnumerable<string> ExtractDetailUrls(HtmlDocument doc)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            // 详情页通常在 /prompt/{slug}，排除列表页本身以及明显的导航/锚点。
    ...
    private bool HasNextPage(HtmlDocument doc, int currentPage)
    {
        var nextLink = doc.DocumentNode.SelectSingleNode("//a[contains(translate(normalize-space(text()), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'next') or contains(normalize-space(text()), '下一页')]");
        if (nextLink is not null)
        {
            return true;
        }

        // 兜底：若页面中仍出现 page=N+1 字样，继续翻页。
        var html = doc.DocumentNode.InnerHtml;
        return Regex.IsMatch(html, $@"page\s*=\s*{currentPage + 1}\b", RegexOptions.IgnoreCase);
    }

    private string ReadTitle(HtmlDocument doc)
    {
    ...
    private string ReadSummary(HtmlDocument doc)
    {
    ...
    private string ReadPromptText(HtmlDocument doc)
    {
        ...
        if (scriptNode is not null)
        {
            ...
            var extracted = ExtractPossiblePromptFromJson(json);
    ...
    private string ReadTags(HtmlDocument doc)
    {
